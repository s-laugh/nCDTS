...
CDTSLoader.getCDTSInstructions(window)
  .then(json => {
    ...
    var allinstructions = [];
    if (
      instructionprocessor[CDTSConstants.CDTS_INSTRUCTIONS_DIRECT_REPLACE_KEY]
    ) {
      allinstructions = allinstructions.concat(
        instructionprocessor[CDTSConstants.CDTS_INSTRUCTIONS_DIRECT_REPLACE_KEY]
      );
    }
    if (
      instructionprocessor[
        CDTSConstants.CDTS_INSTRUCTIONS_COMPONENT_REPLACE_KEY
      ]
    ) {
      allinstructions = allinstructions.concat(
        instructionprocessor[
    ...
    if (
      instructionprocessor[
        CDTSConstants.CDTS_INSTRUCTIONS_ADDITIONAL_SCRIPTS_SRC_KEY
      ]
    ) {
      allinstructions = allinstructions.concat(
        instructionprocessor[
    ...
    Promise.all(allinstructions)
      ...
      .catch(error =>
        console.error(`Error executing CDTS instructions: ${error.message}`)
      );
  })
  .catch(error =>
    console.error(`Error getting CDTS instructions: ${error.message}`)
  );
