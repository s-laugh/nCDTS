...
export default class LoremIpsumComponent extends CDTSBaseComponent {
    ...
    public generateHtml() {
        ...
        if (this.data.output === "ul") {
            let html = "";
            const loremtext = this.getRandomLoremIpsum().split(".");
            let i = this.data.number;
        ...
        if (this.data.output === "hp") {
            ...
            for (let x = 0; x < i; x++) {
                const endposition = LoremIpsumArray[x].indexOf(".");
                const headingtext = LoremIpsumArray[x].substring(0, endposition).trim();
    ...
    public getRandomLoremIpsum() {
        const x = Math.floor((Math.random() * LoremIpsumArray.length));
