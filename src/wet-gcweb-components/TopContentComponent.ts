...
export default class TopContentComponent extends CDTSBaseComponent {
    ...
    constructor(targetElementId, data) {
        ...
        // setting a language property here for now but should be done in
        // CDTSBaseComponent and/or Instruction processor ...
        this.language = "en";
    ...
    public generateSkipNavHtml(lang: string, hasSitemenu: boolean) {
        let html = `
        ...
        if (hasSitemenu) {
            html += `<li class="wb-slc visible-md visible-lg">
            <a class="wb-sl" href="#wb-sec">${TOP_CONTENT_COMPONENT_TEXT.skip_to_section[lang]}</a>
            </li>`;
        }
        html += `</ul>
    ...
    public generateLanguageSelectionHtml(vals) {
        let html = `<section id="wb-lng" class="text-right">
        ...
        if (vals.length === 2) {
            if (Array.isArray(vals[1])) {
                vals[1].forEach((element) => {
                    html += `<li>
                    ${CDTSCommonHtmlGenerators.generateHtmlAnchor(element.url, element.text, element.lang)}</li>`;
                });
            }
        }
        html += `</ul>
    ...
    public generateGoCSearchHtml(vals) {
        return `
        ...
        < form action = "#" method = "post" name = "cse-search-box" role = "search" class="form-inline" >
            <div class="form-group" >
                <label for= "wb-srch-q" class= "wb-inv" >${vals[1]}< /label>
                    < input
                    id = "wb-srch-q"
                    list = "wb-srch-q-ac" class="wb-srch-q form-control"
                    name = "q" type = "search" value = "" size = "34"
            ...
            < div class="form-group submit" >
                ...
                name = "wb-srch-sub" >
                <span class="glyphicon-search glyphicon" > </span><span
    ...
    public generateGoCSiteMenuHtml(vals) {
        return `
    ...
    public generateBreadcrumbHtml(vals) {
        let html = `
        <!-- Breadcrumbs start -->
    ...
    <header>
	    <div id="wb-bnr" class="container">
            ${
            this.data.lngLinks.length > 0 ?
            this.generateLanguageSelectionHtml([TOP_CONTENT_COMPONENT_TEXT.language_selection[this.language]]) : ""
            }
			<div class="row" >
    ...
    public generateHtml() {
        // skip nav
        let html = this.generateSkipNavHtml(this.language, this.data.topSecMenu);
